...
class BaseRepository(Generic[T]):
        ...
        for key, value in filters_by.items():
            if key in ["AND", "OR", "NOT"]:
                ...
                if isinstance(value, list):
                    ...
                    valid_conditions = [c for c in sub_conditions if c is not None]
                    if valid_conditions:
                        if key == "AND":
                            filter_conditions.append(and_(*valid_conditions))
                        elif key == "OR":
                            filter_conditions.append(or_(*valid_conditions))
                        elif key == "NOT":
                            filter_conditions.append(not_(and_(*valid_conditions)))
            elif "__" in key:
    ...
    def _apply_pagination(
        self, query: Select, page: int = 1, page_size: int = 10
    ) -> Select:
    ...
    def _apply_filter_condition(
        ...
        if condition and condition.field and condition.op:
            try:
        ...
        if condition and condition.relation:
            for rel_name, rel_condition in condition.relation.items():
                try:
                    ...
                    raise
        if condition and condition.and_:
    ...
    async def delete(self, id: int, cascade: List[str] = []) -> bool:
