...
@router.get("/taboos")
async def sync_taboos(app_service: AppService = Depends(get_app_service)):
    """Sync taboo data from ArkhamDB"""
    taboos = await app_service.get_taboos()
    return {"status": "completed", "taboos_synced": len(taboos), "data": taboos}


@router.post("/arkhamdb/full")
async def full_arkhamdb_sync(app_service: AppService = Depends(get_app_service)):
