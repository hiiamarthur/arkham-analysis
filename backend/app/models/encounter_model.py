...
class EncounterScenarioSetModel(BaseModel):
    __tablename__ = "encounter_scenario_sets"

    code: Mapped[str] = mapped_column(String(50), primary_key=True, unique=True)
    name: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)
    scenario_codes: Mapped[Optional[List[str]]] = mapped_column(
        JSON, nullable=True
    )  # Scenarios that use this set
...
class EncounterCardModel(BaseModel):
    ...
    code: Mapped[str] = mapped_column(String(50), primary_key=True, unique=True)
    name: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)
    encounter_code: Mapped[Optional[str]] = mapped_column(
        String(50), ForeignKey("encounter_scenario_sets.code"), nullable=True
    )
    ...
    clues_per_investigator: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )
    ...
    # Relationships
    encounter_set: Mapped[Optional[EncounterScenarioSetModel]] = relationship(
        "EncounterScenarioSetModel", back_populates="cards"
    )
...
class ScenarioModel(BaseModel):
    ...
    # Scenario mechanics
    encounter_set_codes: Mapped[Optional[List[str]]] = mapped_column(
        JSON, nullable=True
    )
    ...
    chaos_bag_easy: Mapped[Optional[Dict[str, int]]] = mapped_column(
        JSON, nullable=True
    )
    chaos_bag_standard: Mapped[Optional[Dict[str, int]]] = mapped_column(
        JSON, nullable=True
    )
    chaos_bag_hard: Mapped[Optional[Dict[str, int]]] = mapped_column(
        JSON, nullable=True
    )
    chaos_bag_expert: Mapped[Optional[Dict[str, int]]] = mapped_column(
        JSON, nullable=True
    )
