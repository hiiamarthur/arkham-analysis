...
from app.models.encounter_model import EncounterCardModel, EncounterSetModel, ScenarioModel
...
class ScenarioContextService:
    ...
    async def get_scenario_context(self, scenario_code: str, difficulty: str = "Standard") -> Dict[str, Any]:
        """Get complete scenario context with caching"""
        cache_key = f"{scenario_code}_{difficulty.lower()}"
        ...
        db = next(get_db())
        scenario = db.query(ScenarioModel).filter(ScenarioModel.code == scenario_code).first()
    ...
    async def _build_gathering_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "chaos_tokens": {
                "+1": 2, "0": 3, "-1": 4, "-2": 2, "-3": 1, "-4": 1,
                "skull": 2, "cultist": 1, "tablet": 1, "elder_thing": 1,
                "auto_fail": 1, "elder_sign": 1
            },
            ...
            "encounter_sets": ["torch", "rats", "ghouls", "striking_fear", "ancient_evils"],
            "treachery_count": 8,
    ...
    async def _get_gathering_enemy_stats(self, db: Session) -> Dict[str, Any]:
        ...
        enemy_cards = db.query(EncounterCardModel).filter(
        ...
        if enemy_cards:
            ...
            gathering_enemies = [
                {"name": "Ghoul Minion", "health": 1, "fight": 2, "evade": 3, "damage": 1, "horror": 1},
                {"name": "Swarm of Rats", "health": 1, "fight": 1, "evade": 3, "damage": 1, "horror": 0},
                {"name": "Icy Ghoul", "health": 3, "fight": 4, "evade": 2, "damage": 1, "horror": 1},
                {"name": "Flesh-Eater", "health": 2, "fight": 2, "evade": 2, "damage": 1, "horror": 0}
            ]
            
            total_enemies = len(gathering_enemies)
            avg_health = sum(e["health"] for e in gathering_enemies) / total_enemies  # 1.75
            avg_fight = sum(e["fight"] for e in gathering_enemies) / total_enemies    # 2.25
            avg_evade = sum(e["evade"] for e in gathering_enemies) / total_enemies    # 2.5
    ...
    async def _get_gathering_location_stats(self, db: Session) -> Dict[str, Any]:
        ...
        location_cards = db.query(EncounterCardModel).filter(
        ...
        if location_cards:
            ...
            avg_shroud = sum(card.shroud or 0 for card in location_cards) / total_locations
            avg_clues = sum(card.clues_per_investigator or 0 for card in location_cards) / total_locations
            ...
            avg_shroud = sum(loc["shroud"] for loc in gathering_locations) / len(gathering_locations)  # 1.83
            avg_clues = sum(loc["clues_per_investigator"] for loc in gathering_locations) / len(gathering_locations)  # 1.33
    ...
    def _adjust_for_difficulty(self, context: Dict[str, Any], difficulty: str) -> Dict[str, Any]:
        ...
        if difficulty == "Easy":
            adjusted_context["chaos_tokens"] = {
                "+1": 3, "0": 4, "-1": 3, "-2": 1, "-3": 0, "-4": 0,
                "skull": 2, "cultist": 1, "tablet": 1, "elder_thing": 1,
                "auto_fail": 1, "elder_sign": 1
            }
        elif difficulty == "Hard":
            adjusted_context["chaos_tokens"] = {
                "+1": 1, "0": 2, "-1": 4, "-2": 3, "-3": 2, "-4": 1, "-5": 1,
                "skull": 2, "cultist": 1, "tablet": 1, "elder_thing": 1,
                "auto_fail": 1, "elder_sign": 1
            }
        elif difficulty == "Expert":
            adjusted_context["chaos_tokens"] = {
                "+1": 0, "0": 1, "-1": 3, "-2": 3, "-3": 2, "-4": 2, "-5": 1, "-6": 1,
                "skull": 2, "cultist": 1, "tablet": 1, "elder_thing": 1,
                "auto_fail": 1, "elder_sign": 1
            }
        ...
        if difficulty in ["Hard", "Expert"]:
            if "special_token_effects" in adjusted_context:
                if difficulty == "Hard":
                    adjusted_context["special_token_effects"]["cultist"]["description"] = "-2. If you fail, take 1 horror"
                    adjusted_context["special_token_effects"]["tablet"]["description"] = "-3. If there is a Ghoul enemy at your location, take 1 damage"
                elif difficulty == "Expert":
                    adjusted_context["special_token_effects"]["skull"]["description"] = "-2. If you fail, after this skill test, search the encounter deck and discard pile for a Ghoul enemy, and draw it"
                    adjusted_context["special_token_effects"]["cultist"]["description"] = "Reveal another token. If you fail, take 2 horror"
                    adjusted_context["special_token_effects"]["tablet"]["description"] = "-4. If there is a Ghoul enemy at your location, take 1 damage and 1 horror"
    ...
    async def populate_encounter_cards_from_arkhamdb(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            for card in cards_data:
                ...
                if card.get("type_code") in ["enemy", "treachery", "location"] and card.get("encounter_code"):
                    ...
                    encounter_card = EncounterCardModel(
                        ...
                        traits=card.get("traits", "").split(". ") if card.get("traits") else [],
            ...
            for set_code in encounter_sets:
                existing_set = db.query(EncounterSetModel).filter(EncounterSetModel.code == set_code).first()
                if not existing_set:
                    encounter_set = EncounterSetModel(code=set_code, name=set_code.title())
                    db.add(encounter_set)
            
            # Add all encounter cards
            for card in encounter_cards:
                existing_card = db.query(EncounterCardModel).filter(EncounterCardModel.code == card.code).first()
                if not existing_card:
                    db.add(card)
            
            db.commit()
            print(f"Populated {len(encounter_cards)} encounter cards and {len(encounter_sets)} encounter sets")
    ...
    async def refresh_scenario_cache(self, scenario_code: str = None):
        ...
        if scenario_code:
            ...
            if scenario:
                ...
                db.commit()
                # Clear memory cache
                keys_to_remove = [key for key in self.scenario_cache.keys() if key.startswith(scenario_code)]
